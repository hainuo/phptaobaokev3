...
<header class="mui-bar mui-bar-nav">
    ...
    <h1 class="mui-title">tm优惠券搜索结果</h1>
</header>
  ...
  <div class="mui-row lbd-index-list-head lbd-fixed" id="lbd-index-list-head">
    <ul>
      <li class="{{ $showActive->showActiveForSort($sort, '') }}">
          <a title="tm优惠券搜索结果" class="lbd-a-no-tap" href="{{ route('wx.search.all') }}?q={{ $q }}">精选</a>
      </li>
      <li class="{{ $showActive->showActiveForSort($sort, 'sales') }}">
          <a title="tm优惠券按照销量排序的搜索结果" class="lbd-a-no-tap" href="{{ route('wx.search.all') }}?sort=sales&q={{ $q }}">销量</a>
      </li>
      <li class="{{ $showActive->showActiveForSort($sort, 'commi') }}">
          <a title="tm优惠券按照最热程度排序的搜索结果" class="lbd-a-no-tap" href="{{ route('wx.search.all') }}?sort=commi&q={{ $q }}">最热</a>
      </li>
      <li class="{{ $showActive->showActiveForSort($sort, 'price') }}">
          <a title="tm优惠券按照价格排序的搜索结果" class="lbd-a-no-tap" href="{{ route('wx.search.all') }}?sort=price&q={{ $q }}">价格</a>
      </li>
    </ul>
  </div>
