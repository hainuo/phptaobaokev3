...
<header class="mui-bar mui-bar-nav lbd-index-nav" id="lbd-index-header">
  <div class="mui-scroll-wrapper mui-slider-indicator mui-segmented-control mui-segmented-control-inverted">
        <div class="mui-scroll" id="lbd-index-scroll">
            <a class="mui-control-item mui-active" title="优惠券" href="{{ route('wx.index') }}">推荐</a>
            @foreach($topGoodsCategory as $category)
            <a title="{{ $category->name }}优惠券商品" class="mui-control-item" href="{{  route('goodsCategorys.categoryOne', $category->id) }}">{{ $category->name }}</a>
            @endforeach
        </div>
