...
<div class="mui-row lbd-goods-list" id="lbd-goods-list">
  <ul class="mui-table-view lbd-goods-list-info">
      @foreach($guessYouLikeCoupons as $key => $coupon)
        @if(!empty($coupon->coupon_info))
        <li class="mui-table-view-cell mui-media">
          <a class="addPara" no='data{{ $key }}' href="{{ route('wx.itemInfo.iteminfo') }}/{{ $coupon->num_iid }}">
            <data id="data{{ $key }}" link='{{ $material->getParaStrFromUrl($coupon->coupon_click_url) }}'></data>
            <div class="mui-row">
